    ...
    if (connection === null) {
        ...
        connection.on("Broadcast", function (response) {
            $("#chatRoom").append(
                ...
                '<p class="text-small mb-0 text-white">' + response.messageText + '</p>' +
                '</div>' +
                '<p class="small text-muted">' + response.fromUser + '</p>' +
                '<p class="small text-muted">' + response.messageDt + '</p>' +
                '</div>' +
                '</div>'
            );
        ...
        connection.on("UserConnected", function (response) {
            $("#chatUsers").append(
                ...
                '<h6 class="mb-0">Some User</h6><small class="small font-weight-bold">' + response.connectionDt + '</small>' +
                '</div>' +
